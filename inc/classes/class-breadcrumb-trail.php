...
class Breadcrumb_Trail {
    ...
    public function trail() {
        ...
        if ( 0 < $item_count ) {
            ...
            foreach ( $this->items as $item ) {
                ...
                $item_class = 'trail-item ele-d-inline-flex ele-align-items-center';
                ...
                if ( true === $this->args['show_title'] && $item_count === $item_position ) {
                    ...
                    if ( true === $this->args['show_separator'] ) {
                        $breadcrumb .= sprintf( '<%1$s %2$s>%3$s<span class="ele-separator ele-d-inline-flex">%5$s</span>%4$s</%1$s>', tag_escape( $this->args['item_tag'] ),$attributes, $item, $meta, $this->args['item_sept'] );
                    } else {
            ...
            $breadcrumb = sprintf(
                '<%1$s role="navigation" aria-label="%2$s" class="ele-breadcrumb-trail breadcrumbs %3$s" itemprop="breadcrumb" data-layout="%4$s">%5$s%6$s%7$s</%1$s>',
                tag_escape( $this->args['container'] ),
                esc_attr( $this->labels['aria_label'] ),
                ( true === $this->args['show_separator'] ? 'ele-has-separator' : 'ele-has-no-separator' ),
                esc_attr( $this->args['layout'] ),
                $this->args['before'],
                $breadcrumb,
                $this->args['after']
            );
    ...
    protected function set_labels() {
        ...
        $defaults = array(
            'browse'              => esc_html__( 'Browse:',                               'elementify' ),
            'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'elementify' ),
            'home'                => esc_html__( 'Home',                                  'elementify' ),
            'error_404'           => esc_html__( '404 Not Found',                         'elementify' ),
            'archives'            => esc_html__( 'Archives',                              'elementify' ),
            // Translators: %s is the search query.
            'search'              => esc_html__( 'Search results for: %s',                'elementify' ),
            // Translators: %s is the page number.
            'paged'               => esc_html__( 'Page %s',                               'elementify' ),
            // Translators: %s is the page number.
            'paged_comments'      => esc_html__( 'Comment Page %s',                       'elementify' ),
            // Translators: Minute archive title. %s is the minute time format.
            'archive_minute'      => esc_html__( 'Minute %s',                             'elementify' ),
            // Translators: Weekly archive title. %s is the week date format.
            'archive_week'        => esc_html__( 'Week %s',                               'elementify' ),
    ...
    protected function add_minute_hour_archive_items() {
        ...
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'elementify' ) ) );
    }
    ...
    protected function add_minute_archive_items() {
        ...
        // Add the minute item.
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'elementify' ) ) );
    }
    ...
    protected function add_hour_archive_items() {
        ...
        // Add the hour item.
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'elementify' ) ) );
    }
    ...
    protected function add_day_archive_items() {
        ...
        $year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'elementify' ) ) );
        $month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'elementify' ) ) );
        $day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'elementify' ) ) );
    ...
    protected function add_week_archive_items() {
        ...
        $year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'elementify' ) ) );
        $week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'elementify' ) ) );
    ...
    protected function map_rewrite_tags( $post_id, $path ) {
        ...
        if ( is_array( $matches ) ) {
            ...
            foreach ( $matches as $match ) {
                ...
                if ( '%year%' == $tag )
                    $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'elementify' ) ) ) );

                // If using the %monthnum% tag, add a link to the monthly archive.
                elseif ( '%monthnum%' == $tag )
                    $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'elementify' ) ) ) );

                // If using the %day% tag, add a link to the daily archive.
                elseif ( '%day%' == $tag )
                    $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'elementify' ) ) ) );
